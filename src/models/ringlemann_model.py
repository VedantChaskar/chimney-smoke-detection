"""
Custom CNN Model for Ringlemann Scale Smoke Density Classification

This module contains the RinglemannCNN architecture, a custom convolutional neural
network designed for 6-class smoke density classification based on the Ringlemann scale.
...
class RinglemannCNN(nn.Module):
    """
    Custom CNN for Ringlemann scale smoke density classification
    ...
    # Ringlemann scale class names
    CLASS_NAMES = ['R0', 'R1', 'R2', 'R3', 'R4', 'R5']

    def __init__(self, num_classes: int = 6):
        """
        Initialize the RinglemannCNN model

        Args:
            num_classes: Number of output classes (default: 6 for R0-R5)
        """
        super(RinglemannCNN, self).__init__()
        ...
        self.fc = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(128, num_classes)  # 6-class classification: R0-R5
        )
    ...
    def get_class_names(cls) -> list:
        """
        Get the list of Ringlemann class names

        Returns:
            List of class names ['R0', 'R1', 'R2', 'R3', 'R4', 'R5']
        """
        return cls.CLASS_NAMES.copy()


def load_ringlemann_cnn(model_path: str, device: str = 'cpu') -> RinglemannCNN:
    """
    Load a trained RinglemannCNN model from a checkpoint
    ...
    Returns:
        Loaded RinglemannCNN model in evaluation mode
    """
    model = RinglemannCNN()
...
if __name__ == "__main__":
    ...
    print("RinglemannCNN Model Architecture")
    print("=" * 50)
    print(model)
    print("=" * 50)
